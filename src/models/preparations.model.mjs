...
export async function postPreparation({ data, schema }) {
  ...
  try {
    ...
    if (Object.keys(newRegister).length === 0) {
      response = [
        { status: 400, message: "Missing required fields or not valid data" },
        undefined,
      ];
      return response;
    }
    ...
    const products = items.map((i) =>
      warehouseProducts.find(
        (product) => product.product_type_id == i.product_type_id
      )
    );
    ...
    if (!products.every((p, index) => p.quantity >= items[index].quantity)) {
      response = [
        { status: 400, message: "Not Enought Quantity on Warehouse" },
        undefined,
      ];
      return response;
    }

    const selectedProductType = warehouseProducts.find(
      (p) => p.product_type_id == data.product_type_id
    );
    ...
    products.forEach(async (product, index) => {
      const [err, result] = await postOutput({
        data: {
    ...
    response = [
      ...
      {
        responseQuery,
        dataResponse: { ...data, type: data.product_type_id },
        keyField,
      },
    ];
  } catch (err) {
