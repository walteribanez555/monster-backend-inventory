...
export const handler = async (event) => {
    ...
    const { id, init, end, product_id, warehouse_id, limit, offset  } = typeof( event.queryStringParameters ) === 'object' && Object.keys( event.queryStringParameters ).length > 0 ? event.queryStringParameters : false;
    ...
    try {
        ...
        if ( endpoints.hasOwnProperty( path ) )
            return await endpoints[ path ][ method.toLowerCase() ]( { id, init, end, product_id, warehouse_id,limit, offset, data} );
